...
import { STATION_ABIS } from './chain-info.js';
...
export const makeRawHTMLBroadcast = async (myContract, signer) => {
  ...
  return new Promise((resolve, reject) => {
    let sig;
    const toBroadcast = document.getElementById('raw-html-composition-area').value;
    const parent = document.getElementById('raw-html-hidden-info').getAttribute('parent');
    const tmp = ethers.utils.arrayify(ethers.utils.keccak256(ethers.utils.toUtf8Bytes(toBroadcast)));
...
export const makeJamBroadcast = async (myContract, signer) => {
  ...
  return new Promise((resolve, reject) => {
    let sig;
    const artist = document.getElementById('jam-artist').value;
    const title = document.getElementById('jam-songtitle').value;
    const youtubelink = document.getElementById('jam-youtubelink').value;
    const lyrics = document.getElementById('jam-lyrics').value;
...
export const changeUsername = async (myContract, allUsers) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (Object.values(allUsers).map(({ username }) => { return username; }).includes(newUsername)) { reject(new Error('username is already taken')); }
...
export const changePFPLink = async (myContract, myMetadata) => {
  ...
  return new Promise((resolve, reject) => {
    const tmp = document.getElementById('new-pfp-link-field').value;
    myMetadata.profilePic = tmp;

    const payload = JSON.stringify(myMetadata, null, 2);
...
export const changeRawMetadata = async (myContract, myMetadata) => {
  ...
  return new Promise((resolve, reject) => {
    const payload = document.getElementById('raw-metadata-area').value;
